...
class UsersController {
    constructor() {
        this.userService = new UserManagementService_1.UserManagementService();
    }
    /**
    ...
    async createUser(c) {
        try {
            ...
            if (user.role !== 'admin') {
                return c.json((0, response_1.createErrorResponse)('AUTHORIZATION_ERROR', 'Insufficient permissions'), 403);
            }
            const body = await c.req.json();
            const newUser = await this.userService.createUser(body);
            return c.json((0, response_1.createSuccessResponse)(newUser), 201);
        }
        catch (error) {
    ...
    async deleteUser(c) {
        try {
            ...
            if (user.uid === userId) {
                return c.json((0, response_1.createErrorResponse)('VALIDATION_ERROR', 'Cannot delete your own account'), 400);
            }
            await this.userService.deleteUser(userId, user.uid, user.role);
            return c.json((0, response_1.createSuccessResponse)({ message: 'User deleted successfully' }));
        }
        catch (error) {
    ...
    async updateUserPreferences(c) {
        try {
            ...
            if (user.uid !== userId) {
                return c.json((0, response_1.createErrorResponse)('AUTHORIZATION_ERROR', 'Can only update your own preferences'), 403);
            }
            const preferences = await this.userService.updateUserPreferences(userId, body, user.uid);
            return c.json((0, response_1.createSuccessResponse)(preferences));
        }
        catch (error) {
    ...
    async getUserPreferences(c) {
        try {
            ...
            if (user.uid !== userId) {
                return c.json((0, response_1.createErrorResponse)('AUTHORIZATION_ERROR', 'Can only view your own preferences'), 403);
            }

...
class RealtimeService extends BaseService_1.BaseService {
    constructor() {
        super();
        this.rtdb = (0, database_1.getDatabase)();
        this.db = (0, firestore_1.getFirestore)();
    }
    /**
     * Join or create a real-time editing session
     */
    async joinSession(userId, appId, mode) {
        try {
     ...
     */
    async updateFile(sessionId, userId, fileData) {
        try {
            const sessionRef = this.rtdb.ref(`sessions/${sessionId}`);
     ...
     */
    async sendMessage(sessionId, userId, message, type) {
        try {
            const session = await this.getSession(sessionId, userId);
            if (!session) {
                throw new Error('Session not found');
            ...
            const messageData = {
                ...
                userId,
                userName: (participant === null || participant === void 0 ? void 0 : participant.name) || 'Anonymous',
                message,
                type,
                timestamp: new Date(),
            };
     ...
     */
    async updateCursor(sessionId, userId, fileId, position) {
        try {
            const sessionRef = this.rtdb.ref(`sessions/${sessionId}`);

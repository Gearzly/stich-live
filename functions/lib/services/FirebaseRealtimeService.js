...
class FirebaseRealtimeService extends BaseService_1.BaseService {
    constructor() {
        super(...arguments);
     ...
     */
    async updateProgress(sessionId, status, progress, currentStep, files) {
        const sessionRef = this.database.ref(`generations/${sessionId}`);
        const update = {
            status,
            progress,
     ...
     */
    async completeGeneration(sessionId, files) {
     ...
     */
    async errorGeneration(sessionId, error) {
        const sessionRef = this.database.ref(`generations/${sessionId}`);
        await sessionRef.update({
            status: 'error',
            progress: 0,
     ...
     */
    async simulateGeneration(sessionId, userId) {
        try {
            // Initialize session
  ...
  <React.StrictMode>
    <App />
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                await this.updateProgress(sessionId, 'generating', progress, `Generating ${file.name}...`, files.slice(0, i + 1));
                await this.sendMessage(sessionId, {
                    type: 'file',
                    data: file,
                    timestamp: Date.now(),
    ...
    async cleanupOldSessions(maxAgeHours = 24) {
        ...
        Object.keys(generations).forEach(sessionId => {
            ...
            if (session.timestamp < cutoffTime) {
                deletions.push(this.database.ref(`generations/${sessionId}`).remove(), this.database.ref(`messages/${sessionId}`).remove());
            }
        });
...
exports.FirebaseRealtimeService = FirebaseRealtimeService;

...
const ChatMessageSchema = z.object({
  ...
  sessionId: z.string(),
  agentMode: z.enum(['deterministic', 'smart']).default('deterministic'),
});
...
export class ChatController {
   ...
   */
  static async sendMessage(c: Context): Promise<Response> {
    try {
   ...
   * Get chat session
   */
  static async getChatSession(c: Context): Promise<Response> {
    try {
   ...
   * Save chat session
   */
  static async saveChatSession(c: Context): Promise<Response> {
    try {
      const user = c.get('user');
   ...
   */
  static async listChatSessions(c: Context): Promise<Response> {
    try {
      const user = c.get('user');
   ...
   * Delete chat session
   */
  static async deleteChatSession(c: Context): Promise<Response> {
    try {
      const sessionId = c.req.param('sessionId');
      const user = c.get('user');

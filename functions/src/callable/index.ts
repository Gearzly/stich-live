...
export const generateApp = onCall(
  ...
  async (request: CallableRequest<GenerateAppRequest>) => {
    try {
      ...
      logger.info(`Starting app generation for user: ${userId}`, {
      ...
      logger.error('Error in generateApp:', error);
...
export const deployApp = onCall(
  ...
  async (request: CallableRequest<DeployAppRequest>) => {
    try {
      // Verify authentication
      if (!request.auth) {
        throw new Error('Authentication required');
      ...
      logger.error('Error in deployApp:', error);
...
export const getGenerationStatus = onCall(
  ...
  async (request: CallableRequest<{ generationId: string }>) => {
    try {
      if (!request.auth) {
        throw new Error('Authentication required');
      }
      ...
      // Verify ownership
      if (generationData?.userId !== request.auth.uid) {
 ...
 */
export const cancelGeneration = onCall(
  {
    cors: true,
    enforceAppCheck: false
  },
  async (request: CallableRequest<{ generationId: string }>) => {
    try {
      ...
      if (['completed', 'failed', 'cancelled'].includes(generationData?.status)) {
      ...
      logger.error('Error in cancelGeneration:', error);
